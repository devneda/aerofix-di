...
export default function AddAvionPage() {
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      // Realizamos el POST real a la API
      const response = await fetch(getApiUrl("/aviones"), {
        method: "POST",
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.message || "No se pudo guardar el avión en la base de datos.");
      }

      setSubmitted(true);
      reset();
      // El mensaje de éxito desaparecerá tras 5 segundos
